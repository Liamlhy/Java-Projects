...
import src.edu.grinnell.sortingvisualizer.sortevents.SortEvent;
import src.edu.grinnell.sortingvisualizer.sorts.Sorts;
    ...
    public ControlPanel(final NoteIndices notes, final ArrayPanel panel) {
        ...
        playButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                Integer[] copyArr = Arrays.copyOf(notes.indices, notes.indices.length);
                final List<SortEvent<Integer>> allEvents = generateEvents((String) sorts.getSelectedItem(), copyArr);
                final List<SortEvent<Integer>> events = new LinkedList<SortEvent<Integer>>();
                for (int i = 0; i < allEvents.size(); i++){
                    if (allEvents.get(i).isEmphasized()){
                ...
                timer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        if (index < events.size()) {
                            ...
                            for (int i = 0; i < events.get(index).affectedIndices().size(); i++){
                                scale.playNote(notes.indices[e.affectedIndices().get(i)], e.isEmphasized());
                                notes.highlighted[e.affectedIndices().get(i)] = true;
                            }
                            index++;
                            panel.repaint();
                        } else {
